...
TEST_FIXTURE(SDL_fixture, GameGridOffsets)
{
	CHECK(SDL_init_ok);
	if(SDL_init_ok)
	{
		...
		p_widget->SetOffset(Vector2i(16, 16));
		CHECK_EQUAL(Vector2i(16,16), p_widget->GetOffset());



		p_widget->SetOffset(Vector2i(0, 0));
		last_click_coord = Vector2i(0, 0);
		click_count = 0;
		...
		p_widget->SetOffset(Vector2i(16, 16)); //5*32 + 10 + 16 = 5+26/32
		p_widget->HandleEvent(e);
		CHECK_EQUAL(Vector2i(5, 5), last_click_coord);
		p_widget->SetOffset(Vector2i(24, 16)); //5*32 + 10 + 16 = 6+2/32
		p_widget->HandleEvent(e);
		CHECK_EQUAL(Vector2i(6, 5), last_click_coord);
